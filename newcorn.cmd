...
var keeplist .*
#var keeplist pass|ticket|homespun pouch|brocade pouch|oilcloth pouch|icesilk pouch|hip pouch|voile pouch|war belt|lens|lid|base|pan|sigil|monocle|brooch|niniam wand
...
# Echo script events to this window
var logWindow Log
#var logWindow Conversation
...
action put #echo >%logWindow [corn] Yellow Scarecrow found in room $roomid! when ^You also see.+the Scarecrow
action put #echo >%logWindow [corn] Yellow Harawep's Spider found in room $roomid! when ^You also see.+Harawep's Spider
...
action put #echo >%logWindow [corn] Yellow Scarecrow appears!;put #play NewRank when You hear sinister laughter as the Scarecrow invades the Corn Maze!
action put #echo >%logWindow [corn] Yellow Spider appears!;put #play NewRank when A hissing sound echoes through the Corn Maze as Harawep's Spider makes its appearance!
...
action put #echo >%logWindow [corn] Yellow $1 minutes remaining! when A barefoot Halfling trots up to you and says, "Hey, just to let you know you've only got (.+) minutes of time left.  You can either run to the exit and start over, or use another pass to keep exploring the maze."
...
if_1 then
{
  ...
  if matchre ("%1", "tas") then
  {
    ...
    if %halfling1 > -1 then
    else var halfling1 2
    var halfling2 $halfling2
    if %halfling2 > -1 then
    else var halfling2 104
    var halfling3 $halfling3
    if %halfling3 > -1 then
    else var halfling3 150
    var halfling4 $halfling4
    if %halfling4 > -1 then
    else var halfling4 187
    var halfling5 $halfling5
    if %halfling5 > -1 then
    else var halfling5 227
    var halfling6 $halfling6
    if %halfling6 > -1 then
    else var halfling6 395
    var firstrun 1
    gosub COUNTBOTTLE
...
SCARESPIDER:
  ...
  {
    put #echo >%logWindow [corn] Yellow Scarecrow found in room $roomid!
    put #echo NewRank
    exit
  }
  if matchre ("$roomobjs", "Harawep's Spider") then
  {
    put #echo >%logWindow [corn] Yellow Spider found in room $roomid!
    put #echo NewRank
    exit
  }
  math roomnum add 1
  ...
  {
    put #echo >%logWindow [corn] Yellow Boss not found!
    put #echo NewRank
    exit
  }
  goto SCARESPIDER
...
ABAD:
  ...
  {
    put #echo >%logWindow [corn] Yellow Unable to peform this task!  Please do it manually!
    put #play NewRank
    exit
  }
...
BADPOKE:
  #put #echo >%logWindow [corn] Yellow Error!  Attempted to poke a halfling that wasn't there!
  #put #play NewRank
  #exit
  var badpoke 1
  return
...
FINDHALF:
  ...
  math roomend add 7
  if %roomend > 426 then var roomend 426
  var foundhalf 0
  ...
  else
  {
    ...
    if foundhalf = 0 then
    {
      put #echo >%logWindow [corn] Yellow Unable to find Halfling!  Error!
      put #play NewRank
      exit
    }
  }
...
ROOMTEST:
  if $roomid = 0 then
  {
    ...
    if $roomid = 0 then
    {
      ...
      put #echo >%logWindow [corn] Yellow You are in an unrecognized room and the script is unable to correct!  Move around until the script reacquires a room number and it should take over.
      gosub ROOMRESET
    }
  }
  return
