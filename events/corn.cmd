...
action put #echo >%logWindow Yellow Scarecrow found in room $roomid! when ^You also see.+the Scarecrow
action put #echo >%logWindow Yellow Harawep's Spider found in room $roomid! when ^You also see.+Harawep's Spider
...
if_1 then
{
  ...
  if matchre ("%1", "tas") then
  {
    ...
    var firstrun 1
    gosub COUNTBOTTLE
    put #echo >%logWindow Starting task run with %bottle kernels.
    goto MAINTASKLOOP
  }
...
INCIDENTALSSTART:
  put #echo >%logWindow Starting incidental run.
INCIDENTALS:
  gosub ROOMTEST
  ...
  {
    put #echo >%logWindow Completed incidental run.
    return
  }
  goto INCIDENTALS
...
SEARCH:
  ...
  {
    put #echo >%logWindow Could not find the expected item to search in room %roomlist(%roomnum)!
    return
  }
...
LOOTSORT:
  ...
  {
    put #echo >%logWindow Found %lootitem!
    var stowhand right
    gosub STOW
  }
  else
  {
    put #echo >%logWindow Not keeping %lootitem in $roomid!
...
BADPOKE:
  #put #echo >%logWindow Yellow Error!  Attempted to poke a halfling that wasn't there!
  #put #play NewRank
  #exit
  var badpoke 1
  return
...
FINDHALF:
  ...
  else
  {
    ...
    if foundhalf = 0 then
    {
      put #echo >%logWindow Yellow Unable to find Halfling!  Error!
      put #play NewRank
      exit
    }
  }
...
ROOMTEST:
  if $roomid = 0 then
  {
    ...
    if $roomid = 0 then
    {
      ...
      put #echo >%logWindow Yellow You are in an unrecognized room and the script is unable to correct!  Move around until the script reacquires a room number and it should take over.
      gosub ROOMRESET
    }
  }
  return
...
EXIT:
  gosub COUNTBOTTLE
  put #echo >%logWindow Run ended with %bottle kernels.
  exit
