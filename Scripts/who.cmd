...
####################################################################################################
# CONFIG
## Currently assumes LT will be trained with throwing blades
## TODO: Make that not a thing
...
## Trigger for shoving a bundle into a shadow servant when it is full and making a new one
...
init:
    ...
    ## TODO: Dynamic array sorting of skill learning rates
    initLoop:
...
loop:
    if $monsterdead > 0 then {
        if ($guild = Necromancer && $Thanatology.LearningRate < 33 && "$roomplayers" = "") then gosub perform preserve on %creature
        gosub Skinning
        gosub loot
    }
